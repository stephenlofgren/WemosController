...
class D1MiniChartsViewSet(viewsets.GenericViewSet):
    ...
    def humidity(self, request):
        ...
        for reading in self.queryset:
            mylist.append([ json.dumps(reading.date_reading),
            reading.reading_value])
            if len(mylist) == 5:
                break

        context = {'request': request,
                   'rows': json.dumps(mylist)}
        return render(request, 'WemosController/ReadingCharts.html', context)
